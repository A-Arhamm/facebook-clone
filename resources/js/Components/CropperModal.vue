...
<template>
    <div class="fixed z-50">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex flex-col justify-center min-h-full items-center py-2">
                <div class="transform overflow-hidden rounded-lg bg-white shadow-2xl transition-all max-w-xl">
                    ...
                    <div class="flex items-center bg-white px-4 pb-4">
                        <div>
                            <div class="my-4">
                                <label for="image"
                                    class="flex items-center justify-center font-bold bg-[#E7F3FF] hover:bg-[#D8E7F2] p-2 rounded-lg w-full cursor-pointer text-[#1977F2]">
                                    <Plus :size=20 /> Upload Photo
                                </label>
                                <input type="file" id="image" ref="fileInput" class="hidden" @change="getUploadedImage">
                            </div>
                            <div class="w-[350px] mx-auto">
                                <Cropper class="object-cover" ref="cropper" :stencil-component="CircleStencil"
                                    :src="uploadedImage" />
                            </div>
                            <div class="flex gap-4" :class="uploadedImage ? 'pt-4' : ''">
                                ...
                                <button v-if="uploadedImage" @click="crop" type="button"
                                    class="w-full rounded-md py-2 bg-blue-500 text-white shadow-sm font-bold hover:bg-blue-600 focus:outline-none focus:ring-0">
                                    Crop Image
                                </button>
                            </div>
                        </div>
